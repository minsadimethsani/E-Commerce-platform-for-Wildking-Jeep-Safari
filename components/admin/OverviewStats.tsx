...
interface OverviewStatsProps {
  ...
  isSeeding: boolean;
  setActiveTab: (tab: "bookings" | "inquiries" | "packages" | "fleet") => void;
}
...
export default function OverviewStats({
  ...
  const handleSeedClick = async () => {
    ...
    try {
      await onTriggerSeed();
      setSeedStatusMessage("Database successfully populated/synced with default seed packages & fleet!");
      setTimeout(() => setSeedStatusMessage(null), 5000);
    } catch (err: any) {
      ...
      <div className="bg-gradient-to-r from-emerald-900 to-teal-950 p-6 rounded-2xl border border-emerald-700/50 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-white flex items-center gap-2">
            <span>🛻</span> Executive Safari Dashboard
          </h1>
        ...
        <button
          ...
          disabled={isSeeding}
          className="px-5 py-3 rounded-xl bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-slate-950 font-bold text-sm shadow-lg shadow-amber-500/20 transition-all flex items-center space-x-2 flex-shrink-0"
        >
          <span>{isSeeding ? "⏳ Seeding Database..." : "🌱 Sync / Seed Firestore Data"}</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900/90 border border-emerald-800/40 p-5 rounded-2xl shadow-lg relative overflow-hidden">
          <div className="flex justify-between items-start">
            ...
            <div className="p-3 bg-emerald-500/20 text-emerald-300 rounded-xl text-xl">
              💵
            </div>
          </div>
        ...
        <div className="bg-slate-900/90 border border-emerald-800/40 p-5 rounded-2xl shadow-lg relative overflow-hidden">
          <div className="flex justify-between items-start">
            ...
            <div className="p-3 bg-amber-500/20 text-amber-300 rounded-xl text-xl">
              🐆
            </div>
          </div>
        ...
        <div className="bg-slate-900/90 border border-emerald-800/40 p-5 rounded-2xl shadow-lg relative overflow-hidden">
          <div className="flex justify-between items-start">
            ...
            <div className="p-3 bg-teal-500/20 text-teal-300 rounded-xl text-xl">
              ✉️
            </div>
          </div>
        ...
        <div className="bg-slate-900/90 border border-emerald-800/40 p-5 rounded-2xl shadow-lg relative overflow-hidden">
          <div className="flex justify-between items-start">
            ...
            <div className="p-3 bg-indigo-500/20 text-indigo-300 rounded-xl text-xl">
              🚜
            </div>
          </div>
          ...
          {inquiries.length === 0 ? (
            ...
            <div className="space-y-3">
              {inquiries.slice(0, 4).map((inq, idx) => (
                <div
                  key={inq.id || idx}
                  className="p-3.5 rounded-xl bg-slate-800/60 border border-slate-700/50 space-y-1.5"
                >
                  <div className="flex justify-between items-start">
                    <p className="text-sm font-bold text-emerald-200">{inq.name}</p>
                    <span
                  ...
                  <p className="text-xs text-slate-300 line-clamp-2 italic">"{inq.message}"</p>
